...
public class ZgloszenieTableModel extends AbstractTableModel {
    ...
    private String[] nazwyKolumn = {"Numer zgloszenia","Adres poczatkowy", "Adres koncowy","Data i godzina przyjazdu", "Dodatkowe uwagi dla kierowcy","Status"};
    ...
    public Object getValueAt(int row, int col) {
        ...
        if (col == 1) return listaZgloszen.get(row).getAdresPoczatkowy();
        if (col == 2) return listaZgloszen.get(row).getAdresKoncowy();
        if (col == 3) return listaZgloszen.get(row).getDataGodzinaPrzyjazdu();
        if (col == 4) return listaZgloszen.get(row).getDodatkoweUwagi();
